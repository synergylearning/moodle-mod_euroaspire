...
$url = new moodle_url('/mod/euroaspire/view.php', array('id' => $id));
$PAGE->set_url($url);
...
$assessmentmanager = new \mod_euroaspire\local\control\assessmentmanager($course, $cm, $euroaspire, 'intro');
$assessmentmanager->check_access();
$assessmentmanager->setup_page();

$assessmentmanager->process_intro();

$output = $assessmentmanager->get_renderer();
echo $output->header();
echo $output->assessment_intro($assessmentmanager);
echo $output->footer();
